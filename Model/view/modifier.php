...
include_once '/xampp/htdocs/mbole/connexion.php';
include_once "../Controller/ReclamationC.php";

function getReclamation($id, $pdo)
{
   // $db = config::getconnexion();

    try {
        $query = $pdo->query(
            "SELECT * FROM reclamation_publique where Id_recla='$id'"
        );
        return $query->fetch();
...
if(isset($_GET['id']))
{
    $reclamation = getReclamation($_GET['id'], $pdo);
}

if(isset($_POST['contenu']) && isset($_GET['id']))
{
    $control= new ReclamationC();
    // $newReclamation = new Reclamation("test@gmail.com",$contenu);
    $control->UpdateReclamation($_POST['contenu'],$_GET['id'],$pdo);
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <textarea placeholder="Saisissez votre reclamation ici..." class="form-control" rows="9"
                                name="contenu" id="contenu"> <?php if(isset($_GET['id'])){ echo $reclamation['Contenu'];} ?></textarea>
                        </div>
                    </div>
                    <br>
                    <div class="row">
                        ...
                        <div class="col-sm-6 text-right">
                            <input class="btn btn-action" type="submit" value="Modifier">
                        </div>
                    </div>
                </form>
