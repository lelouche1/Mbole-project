...
function supprimer_utilisateur(int $valeur ,PDO $tab){
    $query=$tab->prepare("DELETE FROM utilisateur WHERE id=:id");
    $query->execute(['id'=>"$valeur"]);
} 

/*function modifier_utilisateur(int $id , $nom, $age, PDO $tab){
    $query=$tab->prepare("UPDATE utilisateur WHERE id=:id");
    $query->execute(['id'=>"$valeur"]);
} */
...
class Utilisateur{
    ...
    function ajouter_utilisateur(PDO $pdo){
        ...
        $reussit = null;
        try{
            $query=$pdo->prepare('INSERT INTO utilisateur(nom, age, email_user, password)
